...
- Rhythmic patterns now accept 1-16 drum pitches. Each pitch has a configurable
  1-7-bit velocity level; lane widths total at most 32 bits. One bit per lane
  is the default, so existing positive binary masks play the same pitches.
...
16 pitches and one velocity bit per pitch. Because the removed pitch and
...
The signed integer sequence format permits all 32 mask bits. A mask using bit
31 is entered as a negative decimal number in two's-complement form. In
